...
def normalizeSymbol(label):
    if label in ['rightarrow', 'sum', 'int', 'pi', 'leq', 'sin','cos','tan', 'lim',
                 'geq', 'infty', 'prime', 'times','alpha', 'beta','pm','log','}','{', 'ln']:
    ...
    if label == '−': label= '-'     
    if label == 'Ω': label= '\\Omega'
    if label == 'ε': label= '\\varepsilon'
    if label == '≧': label= '\\geqq'
    if label == '∉': label= '\\notin'
    if label == '△': label= '\\triangle'
    if label == '⊗': label= '\\otimes'
    if label == '⊥': label= '\\bot'
    if label == '∋': label= '\\ni'
    if label == '↔': label= '\\leftrightarrow'
    if label == 'τ': label= '\\tau'
    if label == '⋅': label= '\\cdot'
    if label == '≡': label= '\\equiv'
    if label == '∪': label= '\\cup'
    if label == '∝': label= '\\propto'
    if label == '□': label= '\\square'
    if label == '▫': label= '\\square'
    if label == '⊕': label= '\\oplus'
    if label == '∵': label= '\\because'
    if label == '⇔': label= '\\Leftrifhtarrow'
    if label == 'ρ': label= '\\rho'
    if label == '∩': label= '\\cap'
    if label == '∧': label= '\\wedge'
    if label == '∠': label= '\\angle'
    if label == '⊂': label= '\\subset'
    if label == '≈': label= '\\approx'
    if label == '⟨': label= '\\langle'
    if label == '∘': label= '\\circle'
    if label == '∴': label= '\\therefore'
    if label == '⊃': label= '\\supset'
    if label == 'ψ': label= '\\psi'
    if label == '≦': label= '\\leqq'
    if label == '∨': label= '\\vee'
    if label == '|': label= '\\vert'
    if label == '⊙': label= '\\odot'
    if label == '∏': label= '\\prod'
    if label == '⋯': label= '\\cdots'
    if label == '⟩': label= '\\rangle'
    if label == '∼': label= '\\sim'
    if label == 'φ': label= '\\varphi'
    if label == ' ̃': label= '\\'
...
def write_mml(mml,filename,outdir):
    ...
    with open(outdir+filename+'.inkml','w',encoding="utf-8") as f:
        f.write(inkml_tags)
        f.write(str(new_mml)+'\n')
        f.write('</math>\n</annotationXML>\n')
        #add groupings
...
if __name__=='__main__':
    ...
    for mmlPath in FILES:
        ...
        #print(name)
        soup = BeautifulSoup(open(mmlPath,encoding="utf-8"),'lxml')
        if soup('math'):
